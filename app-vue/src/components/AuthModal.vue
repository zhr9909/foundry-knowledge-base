<template>
  <div class="auth-overlay" @click.self="$emit('close')">
    <div class="auth-card">
      <button class="auth-close" @click="$emit('close')">&times;</button>
      <div class="auth-tabs">
      ...
      <form v-if="tab === 'login'" @submit.prevent="handleLogin" class="auth-form">
        ...
        <button type="submit" class="auth-btn" :disabled="loading">{{ loading ? '登录中...' : '登录' }}</button>
      ...
      <form v-else @submit.prevent="handleRegister" class="auth-form">
        ...
        <button type="submit" class="auth-btn" :disabled="loading">{{ loading ? '注册中...' : '注册' }}</button>
        <p v-if="error" class="auth-error">{{ error }}</p>
        <div v-if="verifyStep" class="verify-section">
          <p class="verify-hint">验证码已发送到 {{ email }}</p>
          <input v-model="verifyCode" placeholder="输入验证码" maxlength="6" />
          <button type="button" class="auth-btn" @click="handleVerify">验证</button>
        </div>
      </form>
    </div>
  </div>
