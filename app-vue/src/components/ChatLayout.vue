    ...
    <div class="main">
      ...
      <header class="topbar">
        <div class="topbar-left">
          <button v-if="auth.isLoggedIn" class="topbar-btn" @click="newChat" title="????">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          </button>
        </div>
        <div class="topbar-right">
          <button class="topbar-btn" @click="toggleTheme" title="????">
          ...
          <template v-if="auth.isLoggedIn">
            <div class="user-menu">
              <button class="user-avatar-btn" @click.stop="menuOpen = !menuOpen">
                {{ auth.user?.email?.charAt(0).toUpperCase() || 'U' }}
              </button>
              <div v-if="menuOpen" class="user-dropdown" @click.stop>
                <div class="user-dropdown-header">{{ auth.user?.email }}</div>
                <button class="user-dropdown-item" @click="auth.logout(); chat.clearChat(); chat.loadConversations()">????</button>
              </div>
            </div>
          </template>
          <button v-else class="login-btn" @click="auth.showAuth = true">?? / ??</button>
        </div>
      </header>
      <!-- Chat area -->
